...
from src.sheaf_archive_original import SheafArchive, TabuArchive
...
class RiemannianSwarm:
    ...
    def get_agent_curvature(self, agent_idx: int) -> float:
        ...
        - Negative curvature (bridge): Agent is on a bottleneck ΓåÆ boost F to escape
        - Positive curvature (basin): Agent is in a good region ΓåÆ lower F to refine
    ...
    def perform_surgery(self, graph):
        ...
        # === ADAPTIVE THRESHOLD ===
        # Progress: 0.0 (start) ΓåÆ 1.0 (end)
        progress = min(self.generation / max(self.max_generations, 1), 1.0)
        
        # Percentile decays from 5% (aggressive) ΓåÆ 20% (conservative)
